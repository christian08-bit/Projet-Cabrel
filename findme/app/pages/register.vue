...
useSeoMeta({ title: () => t('auth.signupTitle'), robots: 'noindex' })
...
const errors = computed(() => ({
  fullName: touched.fullName ? requiredError(form.fullName, t('auth.fullName')) : null,
  email: touched.email ? emailError(form.email) : null,
  password: touched.password ? passwordError(form.password) : null,
}))

const fieldError = (k: 'fullName' | 'email' | 'password') => errors.value[k] || serverErrors.value[k] || ''
...
async function onSubmit() {
  touched.fullName = touched.email = touched.password = true
  serverErrors.value = {}
  formError.value = ''
...
<template>
  <div>
    <h1 class="font-display text-2xl font-bold text-text-strong">{{ t('auth.signupTitle') }}</h1>
