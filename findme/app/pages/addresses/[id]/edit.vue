...
const { data: initial, error } = await useAsyncData(`address-edit-${id.value}`, () =>
...
<template>
  <AppContainer class="max-w-3xl py-10">
    <NuxtLink :to="localePath(`/addresses/${id}`)" class="mb-6 inline-flex items-center gap-1.5 text-sm font-medium text-text-muted hover:text-brand-500">
      <Icon name="lucide:arrow-left" /> {{ t('common.back') }}
    </NuxtLink>
    <h1 class="mb-8 font-display text-2xl font-bold text-text-strong sm:text-3xl">{{ t('address.editTitle') }}</h1>
    <UiCard v-if="initial">
      <AddressForm
        mode="edit"
        :initial="initial"
